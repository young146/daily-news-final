...
export function buildAffiliateBlockHtml(category, src = 'news') {
  ...
  const cards = slugs.map((s) => {
    ...
    return `<a class="chaovn-aff-btn" href="${url}" target="_blank" rel="nofollow sponsored noopener" style="display:inline-block;margin:5px 8px 5px 0;padding:9px 15px;border:1px solid #e5e7eb;border-radius:22px;background:#ffffff;color:#111827;text-decoration:none;font-size:14px;font-weight:600">${esc(a.label)} &rsaquo;</a>`;
  }).join('');

  // ⚠️ class 를 반드시 함께 넣는다 (2026-08-08 사장님 지적).
  //    웹 브라우저는 인라인 style 만으로 상자를 그려주지만, **앱은 인라인 CSS 를 적용하지 않는다.**
  //    그래서 앱에서는 테두리도 배경도 없이 글자만 나와 본문과 구분이 안 됐다.
  //    앱(react-native-render-html)은 classesStyles 로 클래스별 스타일을 직접 받으므로,
  //    클래스가 있어야 상자 모양을 줄 수 있다. 인라인 style 은 웹용으로 그대로 둔다.
  return `
<div class="chaovn-aff" style="margin:30px 0 8px;padding:16px 18px;border:1px solid #f0e6da;border-radius:12px;background:#fffaf5">
  <div class="chaovn-aff-title" style="font-size:15px;font-weight:700;color:#c2410c;margin-bottom:3px">베트남 생활 &middot; 쇼핑 추천</div>
  <div class="chaovn-aff-sub" style="font-size:12px;color:#9ca3af;margin-bottom:11px">교민이 자주 찾는 서비스 &mdash; 아래에서 바로 확인하세요</div>
  <div>${cards}</div>
  <div class="chaovn-aff-note" style="font-size:11px;color:#b3b3b3;margin-top:11px">* 제휴 링크입니다. 클릭·구매 시 씬짜오의 운영에 도움을 주시게 됩니다. (구매 가격은 동일합니다.)</div>
</div>`.trim();
}
