 ...
 * @param {string} today       'YYYY-MM-DD'
 * @param {object} stats       { [광고제목]: { impressions, clicks } } — 비어 있으면 성과열 생략
 * @param {boolean} statsFailed 조회가 *실패*했는지. 성공했는데 데이터가 0건인 것과 구분한다.
 *                              (둘을 뭉뚱그리면 "고장인가?" 를 매번 다시 확인하게 된다)
 */
export function buildExpiryEmailHtml(items, today, stats = {}, statsFailed = false) {
...
<body style="margin:0;padding:20px;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Malgun Gothic','맑은 고딕',sans-serif;color:#111827">
  <div style="max-width:760px;margin:0 auto;background:#fff;border-radius:12px;padding:22px">
    ...
    <div style="margin-top:18px;padding-top:14px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:12px;line-height:1.7">
      ...
      ${hasStats
        ? '· 노출·클릭은 최근 30일 GA4 집계입니다(웹+앱 합산).'
        : statsFailed
            ? '· ⚠️ 성과 조회에 <b>실패</b>했습니다. 알림은 정상이지만 숫자는 이번 회차에 빠졌습니다.'
            : '· 성과 숫자는 아직 <b>쌓이는 중</b>입니다(집계 시작 2026-08-09). 조회는 정상입니다.'}
    </div>
  </div>
</body></html>`;
}
