...
async function getToken() {
    if (!_authClient) {
        // Vercel 서버리스엔 키 *파일*(GOOGLE_APPLICATION_CREDENTIALS)이 없으므로
        // Firebase Admin 과 동일한 서비스계정 JSON(env)을 재사용해 인증한다.
        ...
        // env 없으면 로컬 ADC/GOOGLE_APPLICATION_CREDENTIALS 로 폴백
...
export async function fetchWeeklyKpis() {
    ...
    // 7) 인기 페이지/화면 Top — 이번주 (구글 리포트의 핵심 정보)
    const pagesResp = await runReport(propertyId, {
        dateRanges: [THIS_WEEK],
    ...
    const pages = (pagesResp.rows || [])
        .map((r) => ({
            ...
            users: Number(r.metricValues[1].value || 0),
            bounceRate: Number(r.metricValues[2].value || 0), // 0~1 비율
        }))
        .filter((p) => p.name && p.name !== '(not set)') // 제목 없는 항목 제외
        .slice(0, 8);

    return { propertyId, totals, installs, channels, platforms, events, dau, pages };
}
...
export function buildReportHtml(kpis) {
    const { totals, installs, channels, platforms, events, dau, pages = [] } = kpis;
    const card = (label, t) => `
    ...
    const clip = (s) => (s && s.length > 42 ? s.slice(0, 42) + '…' : (s || '(이름없음)'));
    const pageRows = pages.length
        ? pages.map((p, i) => {
            const br = (p.bounceRate * 100);
            const brColor = br >= 70 ? '#dc2626' : br >= 40 ? '#d97706' : '#16a34a';
            return `<tr><td style="padding:6px 10px;border-bottom:1px solid #f3f4f6">${i + 1}. ${clip(p.name)}</td><td style="padding:6px 10px;border-bottom:1px solid #f3f4f6;text-align:right">${fmt(p.views)}</td><td style="padding:6px 10px;border-bottom:1px solid #f3f4f6;text-align:right">${fmt(p.users)}</td><td style="padding:6px 10px;border-bottom:1px solid #f3f4f6;text-align:right;color:${brColor}">${br.toFixed(1)}%</td></tr>`;
        }).join('')
        : `<tr><td colspan="4" style="padding:10px;color:#9ca3af">데이터 없음</td></tr>`;
    ...
    <div style="max-width:640px;margin:0 auto;padding:24px">
      ...
      <h2 style="font-size:15px;color:#111827;margin:24px 0 8px">🔥 인기 페이지·화면 Top (이탈률)</h2>
      <table cellpadding="0" cellspacing="0" style="width:100%;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;font-size:13px">
        <tr style="background:#f3f4f6"><th style="text-align:left;padding:8px 10px">페이지 / 화면</th><th style="text-align:right;padding:8px 10px">조회</th><th style="text-align:right;padding:8px 10px">사용자</th><th style="text-align:right;padding:8px 10px">이탈률</th></tr>
        ${pageRows}
      </table>
      <p style="font-size:11px;color:#9ca3af;margin:6px 0 0">이탈률: 낮을수록(초록) 잘 붙잡음, 높을수록(빨강) 들어와서 바로 나감</p>
