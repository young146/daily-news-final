...
// topHeadline 이 있으면 그날의 톱 기사 제목을 앞세운다 — 매일 똑같은 제목은 열어볼 이유가
// 날짜뿐이라 CTR 을 죽인다. 모바일에서 뒷부분이 잘리므로 헤드라인을 앞에 둔다.
export function emailSubject(sponsor, dateString, topHeadline = '') {
  const brand = isSponsored(sponsor) ? sponsor.name : PUBLISHER_NAME;
  if (!topHeadline) return `[${brand}] 데일리뉴스 | ${dateString}`;
  const head = topHeadline.length > 34 ? `${topHeadline.slice(0, 33)}…` : topHeadline;
  return `[${brand}] ${head} | ${dateString}`;
}
