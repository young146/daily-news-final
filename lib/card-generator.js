...
export async function generateCardImageBuffer({
  ...
  // 날짜는 그림에 굽지 않는다 (2026-08-27).
  //   · 뉴스레터에서는 머리에 '데일리뉴스 · 2026년 08월 27일 (목)' 이 크게 들어간다.
  //     바로 아래 그림에 또 날짜가 있으면 같은 말이 두 번이다.
  //   · 카드뉴스로 따로 발행될 때도 글 자체에 발행일이 붙는다.
  //   → 그 자리를 제목이 물려받아, 제목을 위로 올려 더 편히 앉힌다.
  ...
  let line = '';
  let textY = 190;   // 날짜를 뺀 만큼 제목을 위로 올린다
