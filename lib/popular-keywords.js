...
// ── generated 풀을 문자열 배열로 정규화 ({keyword,...} → keyword) ──
function normalizePool(pool, category) {
  const list = pool?.byCategory?.[category];
  if (!Array.isArray(list)) return [];
  return list.map((k) => (typeof k === 'string' ? k : k?.keyword)).filter(Boolean);
}
const weeklyFor = (category) => normalizePool(WEEKLY_KEYWORDS, category); // 네이버 검색량
const gscFor = (category) => normalizePool(GSC_KEYWORDS, category);        // GSC 실유입 검색어

/**
 * 특정 카테고리에 대한 인기 검색어 목록을 반환.
 * 우선순위: GSC(실유입) → 주간 네이버(실측) → baseline → general. 중복 제거 후 max개.
 */
export function getPopularKeywords(category, max = 8) {
  const base = BASELINE[category] || [];
  const merged = [...gscFor(category), ...weeklyFor(category), ...base, ...GENERAL];
  const seen = new Set();
  const out = [];
