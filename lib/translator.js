...
async function callLLM({ system, user, maxTokens = 2000, expectJson = true }) {
  if (PROVIDER === 'anthropic') {
    if (!process.env.ANTHROPIC_API_KEY) {
      throw new Error('ANTHROPIC_API_KEY not set (TRANSLATOR_PROVIDER=anthropic)');
    }
    const userPrompt = expectJson
...
Title: ${item.title}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const raw = await callLLM({ system: SYSTEM_PROMPT_BASE, user: userPrompt, maxTokens: 400 });
...
export async function translateFullArticle(title, summary, content) {
  ...
  // 본문 길이 제한: 베트남 뉴스 평균 길이를 고려해 2500자로 조정.
  // 입력 토큰 감소 + 출력도 자연스럽게 짧아져 1건당 약 20% 시간 단축.
  // 매우 긴 기사는 뒷부분이 잘리지만, 핵심 정보는 기사 앞부분에 집중되므로 큰 손실 아님.
...
Content: ${truncatedContent}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const raw = await callLLM({ system: SYSTEM_PROMPT_BASE, user: userPrompt, maxTokens: 3000 });
      const result = JSON.parse(extractJson(raw));
...
Summary: ${item.summary || ''}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const raw = await callLLM({ system: SYSTEM_PROMPT_BASE, user: userPrompt, maxTokens: 600 });
      const result = JSON.parse(extractJson(raw));
