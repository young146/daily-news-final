...
export async function translateFullArticle(title, summary, content) {
  ...
  const prompt = `
    You are an expert Korean news translator specializing in Vietnamese and English news translation.
    Translate the following news item to authentic Korean news style with MAXIMUM ACCURACY.
    
    **CRITICAL ANTI-HALLUCINATION RULES (MUST FOLLOW):**
    1. Translate EXACTLY what is provided. Do NOT add, remove, or change information.
    ...
    **Korean News Style Guidelines (한국식 기사문 작성):**
    Write in authentic Korean news article style (한국식 기사문). Use formal Korean news language and expressions.
    
    - "translatedTitle": 
      * 15-30 Korean characters, concise and impactful
      * Authentic Korean news headline style (not conversational)
      * Avoid literal word-for-word translation - adapt to Korean conventions
      * **CRITICAL: For ALL proper nouns, include original in parentheses for verification**
        - Vietnamese names: "응우옌반A(Nguyễn Văn A)" - preserve Vietnamese diacritics exactly
        - Vietnamese places: "하노이(Hà Nội)", "호찌민시(Hồ Chí Minh)"
        - Foreign names: "도널드 트럼프(Donald Trump)" if not commonly known
        - Purpose: 번역의 정확성을 확인할 수 있도록 원어 표기 필수
      * **Currency - show original**: "1억 동(100 million VND)", preserve exact numbers
        - Purpose: 화폐 및 금액 번역의 정확성을 확인할 수 있도록 원어 표기 필수
    
    - "translatedSummary": 
      * 2-3 sentences summarizing key points from the input
      * Use Korean news summary style: "~에 따르면", "~한 것으로 나타났다", "~밝혔다"
      * End with "~다", "~했다", "~전망이다" (한국식 기사문 어미)
      * **ALL proper nouns with original**: Preserve Vietnamese names/places exactly with original in parentheses
      * Purpose: 번역의 정확성을 확인할 수 있도록 모든 고유명사에 원어 표기 필수
      * Do NOT add information not in the input summary
    
    - "translatedContent": 
      * Full body text in authentic Korean news reporting style (한국식 기사문)
      ...
      * **CRITICAL: ALL proper nouns MUST include original in parentheses for verification**
        - VIETNAMESE: Preserve diacritics exactly - "응우옌반A(Nguyễn Văn A)", "하노이(Hà Nội)"
        - Preserve Vietnamese spelling exactly as written in input
        - Purpose: 번역의 정확성을 확인할 수 있도록 원어 표기 필수
      * **Currency MUST show original amount**: "1억 동(100 million VND)" - preserve exact numbers
        - Purpose: 화폐 및 금액 번역의 정확성을 확인할 수 있도록 원어 표기 필수
      * IMPORTANT: Return ONLY clean, semantic HTML (<p>, <h3>, <ul>, <li>, <strong>)
      * Do NOT add paragraphs, quotes, or information not in the input content
    
    **Vietnamese Language Specific:**
    - Vietnamese uses diacritics (ă, â, ê, ô, ơ, ư, đ) - preserve them exactly
    - Vietnamese word order may differ - adapt to natural Korean while preserving meaning
    ...
    - DO NOT use names from examples unless they appear in the input text.
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // 본문이 너무 길면 잘라서 처리 (성능 개선)
      const maxContentLength = 5000; // 최대 5000자로 제한
      const truncatedContent = content && content.length > maxContentLength 
        ? content.substring(0, maxContentLength) + "... [본문 일부 생략]"
        : content;
      ...
      const completion = await openai.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "You are an expert Korean news translator specializing in Vietnamese and English news. Your primary responsibility is ACCURACY. Translate ONLY what is provided. Never add, remove, or fabricate information. Preserve all proper nouns exactly as written with original spelling in parentheses. Pay special attention to Vietnamese names and places - preserve diacritics exactly."
          },
...
- Economy: business, finance, trade, real estate, stock market, companies
