...
// ════════════════════════════════════════════════════════════════
/**
 * 기사를 번역·분류하고 **독자 관여도 점수(keywordScore, 0~3)** 를 매긴다.
 *
 * 이 점수가 어디까지 흘러가는지 알고 고칠 것:
 *   크롤링 → keywordScore → 관리화면 후보 정렬(app/admin/page.js)
 *          → 🔥 배지·「명당자리」 추천 → 뉴스레터 배열 순서(app/api/send-daily-email)
 *   즉 **직원이 보는 화면 맨 위에 무엇이 올라오는지를 이 점수가 정한다.**
 *
 * 2026-08-29 — 기준을 두 축으로 넓혔다. 근거는 실측이다:
 *   `npm run newsletter:ctr` 로 재 보니 8/18 과 8/25 만 클릭률이 두 배였다(8.8%, 9.0%).
 *   그런데 **오픈율은 두 날 모두 19% 로 평소와 똑같았다** — 더 많이 열려서가 아니라
 *   연 사람이 더 많이 눌러서다. 즉 제목·발송시각이 아니라 **그날 메일에 무엇이
 *   들었는가**의 문제였다. 두 날의 '끌개' 기사는 각각 760 조회(평소 1위는 ~100):
 *     · 8/18 「외국인 전자신분증(VNeID) 규정 개편」 — 기존 기준에 이미 걸렸다
 *     · 8/25 「호찌민 국제학교 개학 첫날 전격 휴교」 — **기존 기준으로는 0~1점**
 *   후자는 아무도 미리 검색하지 않는 주제다. 검색 수요는 0인데 클릭은 7배였다.
 *   그래서 "평소 검색량은 없어도 **오늘 당장 내 일이 되는 사건**" 축을 따로 넣고,
 *   빠져 있던 교육·의료·은행·교통을 3점 목록에 채웠다.
 *
 *   여행·음식은 사장님 지시로 넣되 **2점 기본**이다. 전부 3점으로 올리면 점수가
 *   평평해져 정렬이 의미를 잃는다. 가격·예약 같은 실용 정보가 있을 때만 3점.
 *
 * 효과는 2~3주 뒤 `npm run newsletter:ctr` 로 확인한다.
 */
...
${keywordBlock(item.category)}
**독자 관여도 점수 (keywordScore) — 두 축으로 본다: ① 평소 검색 수요 ② 오늘 당장의 영향 (중요):**
...
- 1 = 약하게 스침
- 0 = 거의 무관
- **최우선 가점 ① (3점) — 규정·제도가 바뀌어 서류나 절차를 다시 해야 하는 것**:
  비자·체류·출입국 규정, 외국인 소득세·세금, 부동산 규제·집값(호치민/하노이), 법인 설립·사업자 절차,
  노무·법률 변경, 신분증·증명서 제도(전자신분증 등),
  **교육**(국제학교·한국학교 학사일정·휴교·학비·입시), **의료**(병원·건강보험·진료비),
  **은행·송금·환전 규정**, **교통**(공항·항공편·도로·대중교통 변경).
- **최우선 가점 ② (3점) — 평소 검색량은 없어도 "오늘 당장 내 일"이 되는 사건**:
  호치민·하노이에 사는 한국인이나 그 가족이 **그날 일정·계획을 바꿔야 하는 일**이면 3점.
  검색어 목록에 없어도 상관없다 — 아무도 미리 검색하지 않는 종류이기 때문이다.
- **2점 — 수요는 크지만 즉시성은 없는 생활 주제**:
  여행·관광(항공권·숙소·명소), 음식·맛집, 쇼핑·프랜차이즈, 배달앱(Grab/ShopeeFood), 대형몰.
  **단, 가격·예약 방법·주의사항 같이 바로 써먹을 실용 정보가 있으면 3점**으로 올린다.
  (단순 소개·홍보성 기사는 1점 이하)
