...
function headerBlock({ dateString, sponsor, publisherNameEn, isSponsored }) {
  ...
  // 머리에 오렌지를 입힌다 (2026-08-27 사장님 지시).
  // 원래는 흰 바탕이었는데, 그 아래 오렌지 자리는 실제로는 그날의 사진으로 덮인다.
  // 그러면 메일 어디에도 브랜드 색이 남지 않는다 → 머리가 그 자리를 맡는다.
  // 로고(logo-full.png)는 배경이 투명하고 글자가 진해서 오렌지 위에서도 잘 읽히고,
  // 삿갓의 밝은 색이 오히려 살아난다(실측 확인).
  return `
  <tr>
    <td class="pad" bgcolor="${ORANGE}" style="padding:22px 40px 20px;background:${ORANGE};"
        align="center">
      ...
      <div style="font-family:${FONT};font-size:15.5px;font-weight:700;letter-spacing:.01em;
                  color:#FFFFFF;padding-top:11px;">
        데일리뉴스 <span style="color:rgba(255,255,255,.6);font-weight:400;">&nbsp;·&nbsp;</span> ${esc(dateString)}
      </div>
      ${sponsorLine}
    </td>
  </tr>`;
