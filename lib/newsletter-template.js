...
// ── 그때 그 칼럼 — 20년 아카이브 발굴 코너 ────────────────────────────
// (2026-08-30) 본진에 묻힌 사장님 칼럼을 매일 한 편씩 꺼내 보인다.
// 블로그가 아니라 chaovietnam 으로 보낸다 — 원본이 있는 곳이고 애드센스도 거기 있다.
function oldColumnBlock(c) {
  return row(`
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
           style="background:${CREAM};border:1px solid ${LINE};border-radius:16px;">
      <tr>
        <td style="padding:22px 28px 24px;">
          <div style="font-family:${FONT};font-size:11px;font-weight:700;letter-spacing:.2em;
                      color:${MUTE};padding-bottom:10px;">
            그때 그 칼럼 <span style="letter-spacing:0;">&middot; ${esc(c.dateStr)}의 글</span>
          </div>
          <a href="${c.url}" target="_blank"
             style="font-family:${FONT};font-size:16px;line-height:1.5;font-weight:700;
                    color:${HEAD};text-decoration:none;">
            ${esc(c.title)}
          </a>
          ${c.excerpt ? `
          <div style="font-family:${FONT};font-size:13px;line-height:1.7;color:${TEXT};
                      padding-top:8px;">
            ${esc(c.excerpt)}&hellip;
            <a href="${c.url}" target="_blank"
               style="color:${ORANGE_DEEP};text-decoration:none;font-weight:700;">다시 읽기 &rarr;</a>
          </div>` : ''}
        </td>
      </tr>
    </table>`, '12px 40px 0');
...
export function renderDailyNewsEmail(o) {
  const {
    ...
    newsItems = [], koreaNews = [], blogPosts = [], oldColumn = null, promoCards = [],
    brand = {}, baseUrl = 'https://chaovietnam.co.kr',
    // 그날 사정을 담은 인사말 (lib/greeting.js 가 만든다). 없으면 인사 줄이 통째로 안 나온다.
    greetingText = '',
  } = o;

  // 받는 사람 이름이 들어갈 자리. 발송할 때 사람마다 다른 값으로 치환된다.
  // ⚠️ **`esc()` 를 통과시키지 않는다** — 토큰의 `|` 가 이스케이프되면 치환이 안 된다.
  //    이름 자체는 발송 계층에서 넣으므로 여기 사용자 입력이 섞이지 않는다.
  const NAME_SLOT = greetingText ? '{{xcv_name}}' : '';
  ...
  <tr>
    <td class="gutter" align="center" style="padding:16px 10px 28px;">
      ...
      <table role="presentation" class="wrap" width="${MAXW}" cellpadding="0" cellspacing="0" border="0"
        ...
        ${greetingText ? `
        <tr>
          <td class="pad" style="padding:26px 40px 22px;">
            <div style="font-family:${FONT};font-size:17px;line-height:1.7;color:${HEAD};
                        word-break:keep-all;">${NAME_SLOT}${esc(greetingText)}</div>
          </td>
        </tr>` : ''}
        ...
        ${oldColumn ? oldColumnBlock(oldColumn) : ''}
