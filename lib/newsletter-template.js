...
//      버튼은 "모든 것이 한곳에서".
//   ⑭ 자산 안내에 앱 배너 그림(chaovietnam.co.kr/app 의 그것)을 얹었다.
//      '배경 그림 위에 글자' 를 CSS 로 만들지 않은 이유: Outlook 이
//      background-image 를 무시해 글자만 남는다. 그런데 이 그림에는 이미
//      '씬짜오 베트남 / 당신의 베트남 이야기 / 앱 화면' 이 다 들어 있어서,
//      그림을 그대로 얹고 아래에 한 줄과 버튼을 두면 같은 효과가 안전하게 난다.
//
...
// 앱 안내 그림 (chaovietnam.co.kr/app 의 그 배너).
// 원본은 워드프레스에 1200x670 PNG 로 1.2MB — 메일에 그대로 넣으면 휴대폰 데이터로
// 열 때 무겁다. 1000px JPEG 로 줄여(약 100KB) 우리 서버에 두고 그걸 쓴다.
const APP_BANNER_URL = 'https://daily-news-final.vercel.app/app-banner.jpg';
...
function headerBlock({ dateString, sponsor, publisherNameEn, isSponsored }) {
  ...
  <tr>
    <td class="pad" style="padding:22px 40px 18px;background:${PAPER};" align="center">
      ...
      <div style="font-family:${FONT};font-size:15.5px;font-weight:700;letter-spacing:.01em;
                  color:${HEAD};padding-top:11px;">
        데일리뉴스 <span style="color:${ORANGE};font-weight:400;">&nbsp;·&nbsp;</span> ${esc(dateString)}
      </div>
      ${sponsorLine}
    </td>
  </tr>`;
      ...
      <tr>
        <td>
          <a href="${href}" target="_blank" style="display:block;">
            <img src="${APP_BANNER_URL}" alt="씬짜오베트남 앱 — 당신의 베트남 이야기"
                 width="${MAXW - 80}"
                 style="width:100%;height:auto;display:block;border:0;
                        border-radius:16px 16px 0 0;" />
          </a>
