...
async function sendBccWithAccount(transporter, senderEmail, toEmails, subject, htmlContent, batchSize, label) {
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        try {
            ...
            failed += batch.length;
            console.error(`  ❌ [${label}] 배치 ${i + 1}/${batches.length} 실패:`, err.message);
        }
    }
    return { succeeded, failed, batchTotal: batches.length };
}
...
export async function sendNewsletterBccBatched(toEmails, subject, htmlContent, batchSize = 100) {
    ...
    const mid = hasAccount2 ? Math.ceil(toEmails.length / 2) : toEmails.length;
    const list1 = toEmails.slice(0, mid);
    const list2 = hasAccount2 ? toEmails.slice(mid) : [];

    console.log(`[SMTP] 총 ${toEmails.length}명 분산 발송 시작`);
    console.log(`  계정1 (${process.env.SMTP_USER}): ${list1.length}명`);
    if (hasAccount2) console.log(`  계정2 (${process.env.SMTP_USER2}): ${list2.length}명`);

    // 계정1 transporter
    const t1 = createSmtpTransporter();
    await t1.verify();
    ...
    // 두 계정 병렬 발송
    const tasks = [
        sendBccWithAccount(t1, process.env.SMTP_USER, list1, subject, htmlContent, batchSize, '계정1'),
    ];
    ...
    results.forEach((r, idx) => {
        if (r.status === 'fulfilled') {
            ...
            console.error(`[계정${idx + 1}] 전체 실패:`, r.reason?.message);
            totalFailed += idx === 0 ? list1.length : list2.length;
        }
    });

    console.log(`[SMTP DONE] 총 ${totalBatches}배치 | 성공 ${totalSucceeded}명 | 실패 ${totalFailed}명`);
    return { batchTotal: totalBatches, succeeded: totalSucceeded, failed: totalFailed };
}
