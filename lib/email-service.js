...
// 받는 사람 이름을 끼워 넣을 자리표.
//
// 🔴 2026-09-04 실패에서 배운 것 — 처음엔 `-|이름|-` 로 잡았는데 **치환이 안 되고
//    토큰이 메일에 그대로 찍혔다.** 원인은 `@sendgrid/mail` 이 치환 키를 자동으로
//    `{{ }}` 로 감싸기 때문이다 (substitutionWrappers 기본값):
//        우리가 넘긴 키   '-|이름|-'
//        실제 전송 태그   '{{-|이름|-}}'   ← 감싸짐
//        HTML 안의 글자   '-|이름|-'       ← 안 맞아서 그대로 남는다
//    그래서 **라이브러리가 만드는 모양에 HTML 을 맞춘다** — 키는 `xcv_name`,
//    본문에 넣는 자리표는 `{{xcv_name}}`.
export const NAME_KEY = 'xcv_name';
export const NAME_TOKEN = `{{${NAME_KEY}}}`;
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        try {
            ...
            const personalizations = batch.map(email => {
                ...
                //
                // 이름은 **굵고 진하게** 보이게 한다 (2026-09-04 사장님: "특히 이름은 분명하게").
                // ⚠️ 치환값에 태그를 넣으므로 이름에 `<` `>` `&` 가 있으면 서식이 깨진다 —
                //    이름은 lib/subscriber-name.js 가 한글·로마자만 통과시키지만, 만약을
                //    대비해 여기서 한 번 더 막는다.
                const nm = (nameByEmail && nameByEmail[email.toLowerCase()]) || '';
                const safe = nm.replace(/[<>&"]/g, '');
                const greet = safe
                    ? `<strong style="color:#231A14;font-weight:700;">${safe}</strong>, `
                    : '';
                return {
                    to: [{ email }],
                    ...
                    //    본문 아무 데나 걸려서 글이 깨진다.
                    // 키만 넘긴다 — 라이브러리가 `{{xcv_name}}` 으로 감싸 전송한다
                    substitutions: { [NAME_KEY]: greet },
                    headers: {
                        'List-Unsubscribe':
