...
const MAX_BCC_PER_BATCH = 50; // 구글 스팸 필터를 회피하기 위해 50명 이하로 엄격하게 제한
...
export async function sendNewsletterBatchedBccSmtp(toEmails, subject, htmlContent, smtpAccount = 'both') {
    ...
    // 이메일을 MAX_BCC_PER_BATCH단위 묶음(Batch) 배열로 쪼개기
    const batches = [];
        ...
        while (!sent && currentAccIdx < accounts.length) {
            ...
            try {
                await activeTransporter.sendMail({
                    ...
                    replyTo: `"${FROM_NAME}" <info@chaovietnam.co.kr>`,
                    to: acc.user, // To는 자기 자신, 실수신자는 BCC
                    bcc: batchEmails,
                    subject,
                    text: `씬짜오베트남 데일리뉴스\n\n뉴스 터미널에서 전체 뉴스를 확인하세요:\nhttps://chaovietnam.co.kr/daily-news-terminal/\n\n수신 거부(Unsubscribe): ${baseUrl}/unsubscribe`, // 플레인 텍스트 주가로 스팸 등점 빠집니다
                    html: htmlContent,
                    headers: {
                        // 대량 뉴스레터 표준 헤더 (ISP가 스팸이 아닌 뉴스레터로 인식하게 돵)
                        'Precedence': 'bulk',
                        'X-Auto-Response-Suppress': 'OOF, DR, RN, NRN, AutoReply',
                        'List-Unsubscribe': `<mailto:info@chaovietnam.co.kr?subject=unsubscribe>, <${baseUrl}/unsubscribe>`,
                ...
                // 묶음 사이 2~5초 랜덤 대기하여 구글 스팸 필터(기계적 발송 패턴 인지) 회피
