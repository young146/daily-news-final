...
// ─── 1. SendGrid 배치 발송 ────────────────────────────────────────────────────
export async function sendNewsletterSendGrid(toEmails, subject, htmlContent, campaignId) {
...
export async function sendNewsletterWithFallback(toEmails, subject, htmlContent, options = {}) {
    const { smtpAccount = 'both', campaignId } = options;
    ...
    if (process.env.SENDGRID_API_KEY) {
        try {
            ...
            result = await sendNewsletterSendGrid(toEmails, subject, htmlContent, finalCampaignId);
        ...
        console.log(`[이메일] SENDGRID_API_KEY 없음. SMTP BCC로 발송... (캠페인: ${finalCampaignId})`);
        try {
            result = await sendNewsletterBatchedBccSmtp(toEmails, subject, htmlContent, smtpAccount, finalCampaignId);
            method = 'smtp';
        } catch (smtpErr) {
