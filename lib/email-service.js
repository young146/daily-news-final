...
// ─── 4. SMTP BCC 듀얼 계정 발송 (두 계정으로 절반씩 병렬 발송) ───────────────
export async function sendNewsletterBccBatched(toEmails, subject, htmlContent, batchSize = 100, smtpAccount = 'both') {
    ...
    if (smtpAccount === 'account2' && hasAccount2) {
        list2 = toEmails;
    } else if (smtpAccount === 'account3' && hasAccount3) {
        list3 = toEmails;
    } else if (smtpAccount === 'account1') {
        list1 = toEmails;
    } else {
        // 'both': 사용 가능 계정 수에 따라 균등 분산
        const activeAccounts = 1 + (hasAccount2 ? 1 : 0) + (hasAccount3 ? 1 : 0);
        const chunkSize = Math.ceil(toEmails.length / activeAccounts);
        list1 = toEmails.slice(0, chunkSize);
        if (hasAccount2) list2 = toEmails.slice(chunkSize, chunkSize * 2);
        if (hasAccount3) list3 = toEmails.slice(hasAccount2 ? chunkSize * 2 : chunkSize);
    }

    console.log(`[SMTP] 총 ${toEmails.length}명 발송 시작 (계정: ${smtpAccount})`);
    ...
    if (list1.length > 0) {
        ...
        try {
            await t1.verify();
            console.log('[SMTP] 계정1 연결 확인 완료');
        } catch (e) {
            console.warn('[SMTP] 계정1 verify 실패 (발송은 계속 시도):', e.message);
        }
    }

    // 계정2 transporter
    let t2 = null;
    if (list2.length > 0 && process.env.SMTP_USER2) {
        t2 = nodemailer.createTransport({
            host: 'smtp.gmail.com', port: 465, secure: true,
            auth: { user: process.env.SMTP_USER2, pass: process.env.SMTP_PASS2 },
        });
        try {
            await t2.verify();
            console.log('[SMTP] 계정2 연결 확인 완료');
        } catch (e) {
            console.warn('[SMTP] 계정2 verify 실패 (발송은 계속 시도):', e.message);
        }
    }

    // 계정3 transporter
    let t3 = null;
    if (list3.length > 0 && process.env.SMTP_USER3) {
        t3 = nodemailer.createTransport({
            host: 'smtp.gmail.com', port: 465, secure: true,
            auth: { user: process.env.SMTP_USER3, pass: process.env.SMTP_PASS3 },
        });
        try {
            await t3.verify();
            console.log('[SMTP] 계정3 연결 확인 완료');
        } catch (e) {
            console.warn('[SMTP] 계정3 verify 실패 (발송은 계속 시도):', e.message);
        }
    }

    // 계정들 병렬 발송
    const tasks = [];
    if (t1 && list1.length > 0)
        tasks.push(sendBccWithAccount(t1, process.env.SMTP_USER, list1, subject, htmlContent, batchSize, '계정1'));
    if (t2 && list2.length > 0)
        tasks.push(sendBccWithAccount(t2, process.env.SMTP_USER2, list2, subject, htmlContent, batchSize, '계정2'));
    if (t3 && list3.length > 0)
        tasks.push(sendBccWithAccount(t3, process.env.SMTP_USER3, list3, subject, htmlContent, batchSize, '계정3'));

    if (tasks.length === 0) {
        return { batchTotal: 0, succeeded: 0, failed: toEmails.length, note: '선택된 계정에 수신자가 없음' };
    }
    ...
    results.forEach((r, idx) => {
        if (r.status === 'fulfilled') {
            ...
            totalBatches += r.value.batchTotal;
            if (r.value.errors?.length) allErrors.push(...r.value.errors);
        } else {
            const msg = `계정${idx + 1} 전체 실패: ${r.reason?.message}`;
            console.error(`[${msg}]`);
            allErrors.push(msg);
            totalFailed += idx === 0 ? list1.length : list2.length;
        }
    });
