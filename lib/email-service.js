...
export async function sendNewsletterIndividual(toEmails, subject, htmlContent, smtpAccount = 'both') {
    ...
    const setupTransporter = async (acc) => {
        const t = nodemailer.createTransport({
            ...
            pool: true,           // ✅ 연결 재사용 (이메일마다 재로그인 방지)
            maxConnections: 1,    // 계정당 연결 1개 유지
    ...
    for (let i = 0; i < toEmails.length; i++) {
        ...
        while (!sent && currentAccIdx < accounts.length) {
            ...
            try {
                ...
                // 할당량 초과, 인증 오류, 로그인 실패 시 계정 전환
                if (
                    err.message.includes('5.4.5') || err.message.includes('quota') ||
                    err.message.includes('limit') || err.message.includes('Auth') ||
                    err.message.includes('credentials') || err.message.includes('Too many') ||
                    err.message.includes('Invalid login') || err.message.includes('454')
                ) {
                    console.error(`  🚨 [${acc.label}] 발송 제한/인증 오류: ${err.message}. 다음 계정으로 전환합니다.`);
                    // pool 모드에서는 transporter 닫고 새로 생성
                    try { activeTransporter.close(); } catch (_) { }
                    currentAccIdx++;
                    sentCountThisAccount = 0;
                    if (currentAccIdx < accounts.length) {
