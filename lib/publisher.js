...
export async function publishToMainSite(item) {
  ...
  try {
    ...
    // 제휴 추천 블록 — 관련 카테고리 기사 하단에 정보형으로 첨부(하드뉴스 제외).
    // nofollow sponsored + 제휴 고지 포함. 생성 실패해도 발행은 계속(try/catch).
    try {
      const affBlock = buildAffiliateBlockHtml(item.category, "news");
      if (affBlock) finalContent = finalContent + affBlock;
    } catch (affErr) {
      console.warn("[Publish] 제휴 블록 생성 실패(무시):", affErr.message);
    }
