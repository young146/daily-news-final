...
const SSL_PROBLEM_HOSTS = ['img3.yna.co.kr', 'img.yna.co.kr', 'yna.co.kr', 'vnanet.vn'];
...
<div class="news-body-content">`;
        ...
        const isTopNewsValue = item.isTopNews === true || item.isTopNews === 'true' || item.isTopNews === 1 || item.isTopNews === '1';
        postData.meta.is_top_news = isTopNewsValue ? '1' : '0';
        console.log(`[Publish] Setting is_top_news: ${postData.meta.is_top_news} (from item.isTopNews: ${item.isTopNews}, type: ${typeof item.isTopNews})`);
...
export async function publishToDailySite(item) {
    ...
    try {
        ...
        const sourceAttribution = generateSourceAttribution(item);
        let summaryContent = sourceAttribution + (item.translatedSummary || item.summary || '');
...
export async function publishCardNewsToWordPress(imageBuffer, date, options = {}) {
    ...
    try {
        ...
        if (!uploadResponse.ok) {
            ...
            throw new Error(`Image upload failed: ${errorPayload?.message || uploadResponse.status} (${String(errorPayload).slice(0, 200)})`);
        }
        if (!uploadContentType.includes('application/json')) {
            const raw = await uploadResponse.text().catch(() => '');
            throw new Error(`Image upload response was not JSON. Check WP_URL/WP_USER/WP_PASSWORD. Payload preview: ${raw.slice(0, 200)}`);
        }
        ...
        if (!pagesResponse.ok) {
            ...
            throw new Error(`Failed to fetch News Terminal page: ${errorPayload?.message || pagesResponse.status}`);
        }
        if (!pagesContentType.includes('application/json')) {
            const raw = await pagesResponse.text().catch(() => '');
            throw new Error(`Pages API returned non-JSON. Payload preview: ${raw.slice(0, 200)}`);
        }
        ...
        if (!updateResponse.ok) {
            ...
            console.log(`[CardNews] Page update response:`, JSON.stringify(errorData));
