...
export async function publishToMainSite(item) {
    ...
    try {
        ...
        // Add source attribution at the beginning
        const sourceAttribution = `<p style="color: #666; font-size: 14px; margin-bottom: 20px; padding: 10px; background: #f5f5f5; border-left: 3px solid #007bff;">📰 <strong>출처:</strong> ${item.source || 'Unknown'}${item.originalUrl ? ` | <a href="${item.originalUrl}" target="_blank" rel="noopener noreferrer">원문 보기</a>` : ''}</p>`;
...
export async function publishCardNewsToWordPress(imageBuffer, date, options = {}) {
    ...
    try {
        ...
        // 1. Upload the image to WordPress media library (JPEG for better Zalo/Kakao compatibility)
