...
export async function publishCardNewsToWordPress(
  ...
  try {
    ...
    // 날짜 파라미터 추가 (MMDD 형식)
    let baseTerminalUrl = updatedPage.link || options.terminalUrl || `${WP_URL}/daily-news-terminal/`;
    let terminalUrl = baseTerminalUrl;
    
    // date 파라미터가 있으면 날짜 파라미터 추가 (YYYY-MM-DD 형식에서 MMDD 추출)
    if (date) {
      try {
        ...
        if (dateMatch) {
          ...
          const dateParam = `${month}${day}`; // MMDD 형식
          // URL에 이미 쿼리 파라미터가 있는지 확인
          const separator = baseTerminalUrl.includes('?') ? '&' : '?';
          terminalUrl = `${baseTerminalUrl}${separator}v=${dateParam}`;
        }
      } catch (error) {
        console.warn(`[CardNews] 날짜 파라미터 파싱 실패, 기본 URL 사용:`, error);
      }
    }
