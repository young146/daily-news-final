// (2026-08-21) 제휴 블록은 본문에 넣지 않는다 — 통합 광고센터가 담당. import 제거.
// import { buildAffiliateBlockHtml } from "./affiliate-block.js";
...
/**
 * 임의 이미지 버퍼를 워드프레스 미디어로 업로드 (콘텐츠 타입/확장자 보존).
 * 로고처럼 PNG 투명도가 중요한 이미지용. uploadImageDirectly 는 jpeg 강제라 부적합.
 * @param {Buffer} buffer - 이미지 바이트
 * @param {string} filename - 확장자 제외 파일명
 * @param {string} contentType - 예: 'image/png', 'image/jpeg'
 * @returns {Promise<{id:number,url:string}|null>}
 */
export async function uploadMediaToWordPress(buffer, filename, contentType = "image/png") {
  if (!buffer || !WP_PASSWORD) return null;
  try {
    const auth = getWPAuth();
    ...
    if (!response.ok) {
      ...
      console.log("[Image] Media upload failed:", err.message || response.status);
      return null;
    }
    const result = await response.json();
    console.log(`[Image] ✅ Media upload success! ID: ${result.id}`);
    return { id: result.id, url: result.source_url };
  } catch (error) {
    console.error("[Image] Media upload error:", error.message);
    return null;
  }
}
...
function sanitizeForWordPress(str) {
  ...
  str = str.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "");
  // 짝이 맞지 않는(홀로 남은) 서로게이트 제거 — 남아 있으면 WordPress REST가
  // rest_invalid_json(400)으로 발행을 거부한다.
  str = str.replace(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])/g, "");
  str = str.replace(/(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/g, "");
...
/**
 * Rank Math용 SEO 메타 설명 생성.
 * 요약문(출처·날짜 군더더기 없는 깔끔한 텍스트)에서 HTML 제거·공백 정리 후 ~160자에서
 * 단어 경계로 자른다. 이걸 rank_math_description 으로 넣으면 구글 검색 설명과 소셜 공유
 * 설명(og:description)이 일치하고, 본문 앞 "출처: … 날짜: …" 군더더기가 사라진다.
 */
function buildSeoDescription(item) {
  let s = item.translatedSummary || item.summary || item.translatedTitle || item.title || "";
  s = String(s).replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  s = sanitizeForWordPress(s);
  const MAX = 160;
  if (s.length > MAX) {
...
export async function publishToMainSite(item) {
  ...
  try {
    ...
    // ⛔ 제휴 블록을 본문에 넣지 않는다 (2026-08-21 방침 변경).
    //    왜: 본문 HTML 에 광고를 박으면 ① 카테고리마다 누락이 생기고 ② 디자인을 바꿀 때마다
    //    이미 발행된 수만 건을 전부 고쳐야 한다(실제로 1.6만 건 정리 사태). 회사 표준은
    //    **통합 광고센터(ads_unified) → 공개 API → 각 지면이 슬롯에 자동 삽입**이다.
    //    (워드프레스: xinchao-unified-ads 플러그인 / vnkorlife·앱: 같은 API를 읽음)
    //    제휴 상품도 통합센터에 등록해 광고와 같은 경로로 내보낸다.
    //    → lib/affiliate-block.js 는 더 이상 발행 경로에서 쓰지 않는다.
    ...
    // Rank Math SEO 설명 명시 세팅 → 검색/소셜 설명 일치 + "출처/날짜" 군더더기 제거.
    // (제목은 Rank Math 템플릿이 "제목 - 브랜드"로 이미 잘 처리하므로 건드리지 않음)
    // rank_math_description 은 mu-plugin(rankmath-rest-meta.php)이 REST에 열어줘야 저장됨.
    const seoDescription = buildSeoDescription(item);
    if (seoDescription) {
      postData.meta.rank_math_description = seoDescription;
      console.log(`[Publish] Setting rank_math_description (${seoDescription.length}자): ${seoDescription.substring(0, 50)}...`);
    }
    // Rank Math 포커스 키워드 — "이 기사를 어떤 검색어로 최적화할지" RankMath에 알려주는 1순위 신호.
    // 크롤 단계(translateAndCategorize)에서 이미 산출·저장한 matchedKeyword(가장 잘 맞는 인기
    // 검색어 1개)를 그대로 사용. 값이 없으면(무관 기사) 세팅 안 함 — 틀린 키워드보다 없는 게 낫다.
    // rank_math_focus_keyword 는 mu-plugin(rankmath-rest-meta.php)이 REST에 열어줘야 저장됨.
    if (item.matchedKeyword) {
      postData.meta.rank_math_focus_keyword = item.matchedKeyword;
      console.log(`[Publish] Setting rank_math_focus_keyword: ${item.matchedKeyword}`);
    }
...
export async function publishCardNewsToWordPress(
  ...
  try {
    ...
    // 1. Upload the image to WordPress media library (JPEG — 카카오톡 og:image 500KB 한계 회피)
    const filename = `daily-news-card-${date}`;
    console.log(`[CardNews] Uploading image with filename: ${filename}.jpg`);
