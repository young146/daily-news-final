...
// 매거진 "증분" 갱신 — 최근 N일 안에 작성·수정된 글만 받아 upsert.
// 전체 재색인(buildMagazine, 7000여건·약 6분)은 Vercel 크론 300초 제한을 넘으므로,
// 매일 크론은 이 함수로 신규/수정분만 빠르게 반영한다. (전체 재색인은 CLI로 가끔 수동 실행)
async function buildMagazineRecent(prisma, days = 14) {
  if (!WP_PASSWORD) return { upserted: 0, skipped: 'no WP password' };
  const catMap = await fetchCategoryMap();
  const cutoff = new Date(Date.now() - days * 86400000).toISOString().slice(0, 19);
  // 작성 후(after)·수정 후(modified_after) 둘 다 — 새 글과 고친 글 모두 반영
  const posts = await fetchWpPosts(
    `categories_exclude=${DAILY_NEWS_CAT}&after=${cutoff}&modified_after=${cutoff}&orderby=modified`,
    0, 'id,link,date,title,excerpt,content,categories', 50);
  if (posts === null) return { upserted: 0 };
  let n = 0;
  for (const p of posts) {
    const rec = wpToRecord(p, 'magazine', { catMap, includeBody: true });
    const { id, ...rest } = rec;
    await prisma.searchIndex.upsert({ where: { id }, create: rec, update: rest });
    n++;
  }
  return { upserted: n, days };
}
...
async function buildCompany(prisma) {
  ...
  const records = all.map(c => {
    ...
    // ── 원본에 있는데 색인에서 빠져 있던 항목들 (2026-08-08 실측 후 추가) ──────────
    // 표본 500건 기준 채워진 비율:
    //   employees 58.2% · products 55.0% · founded_year 2.2% · description 2.4%
    // products 는 원래 들어가 있었고, 나머지 셋이 빠져 있었다.
    //
    // ⚠️ founded_year 는 2.2% 뿐이라 **정렬·필터 기준으로 쓰면 안 된다.**
    //    97.8% 가 빈 값인데 연도순으로 줄 세우면, "오래된 기업"을 물었을 때 값이 있는
    //    소수만 나오고 실제로 더 오래된 기업 대부분이 빠진다 — 답이 아니라 착시다.
    //    검색 대상에만 넣어 "있는 곳은 걸리게" 한다.
    // ⚠️ employees 는 "한국인 1명, 베트남인 6명" 같은 **자연어**다. 숫자 비교는 못 하지만
    //    "한국인 직원" 같은 검색에는 그대로 걸린다.
    const year = String(c.founded_year || '').trim();
    const emp = String(c.employees || '').trim();
    // 연도는 표기를 늘려 둔다 — "1997" 로 쳐도, "1997년" 으로 쳐도, "설립" 으로도 걸리게.
    const yearText = year ? `${year} ${year}년 설립 창립` : '';

    const parts = [
      name, c.director, c.industry_group, c.industry_detail, c.area, koCity, c.address,
      c.products, c.description, yearText, emp, c.search_text,
    ].filter(Boolean).join(' ');

    return {
      id: `company:${c.id}`, type: 'company', title: name,
      // 목록에서 바로 판단할 수 있게 설립연도·직원수를 요약에 노출한다. 없으면 안 붙는다(지어내지 않는다).
      summary: [c.industry_group, koCity, year ? `${year}년 설립` : null, emp || null, c.address]
...
module.exports = {
  ...
  // builders
  buildNews, buildMagazine, buildMagazineRecent, buildCompany, fetchNeighbor, neighborToRecord, refreshNeighbor, buildYellow,
};
