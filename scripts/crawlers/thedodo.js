...
const BASE_URL = 'https://www.thedodo.com';
...
async function crawlTheDodo() {
    ...
    try {
        ...
        // Find article links
        $('a[href*="/story/"], a[href*="/article/"], article a, .story-card a, .article-card a').each((index, element) => {
            ...
            const title = $(element).text().trim() || $(element).find('h2, h3, .title').text().trim();
            if (!title || title.length < 20 || title.length > 200) return;

            const container = $(element).closest('article, .story-card, .article-card, .card') || $(element).parent();
        ...
        for (const item of listItems) {
            try {
                ...
                if (!content || content.length < 100) {
                    const paragraphs = $detail('article p, .article-body p, .story-content p').map((i, el) => $detail(el).html()).get();
                ...
                if (!item.imageUrl) {
                    item.imageUrl = $detail('meta[property="og:image"]').attr('content') || 
                                   $detail('.article-image img, .story-image img, article img').first().attr('src') || '';
                }
        ...
        console.log(`The Dodo: ${detailedItems.length} items with details`);
        return detailedItems;
    } catch (error) {
        console.error('Error crawling The Dodo:', error.message);
        return [];
    }
}
