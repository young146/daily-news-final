...
async function crawlVnExpressTravel() {
    console.log('[VnExpress Travel] Starting crawl...');
    try {
        ...
        console.log('[VnExpress Travel] Parsing list items...');
        ...
        console.log(`[VnExpress Travel] Found ${listItems.length} list items`);
        ...
        const fetchDetail = async (item) => {
            try {
                ...
                console.error(`[VnExpress Travel] Failed: ${item.title.substring(0, 30)}...`, err.message);
        ...
        for (let i = 0; i < listItems.length; i += BATCH_SIZE) {
            ...
            console.log(`[VnExpress Travel] Processing batch ${Math.floor(i / BATCH_SIZE) + 1} (${batch.length} items)...`);
        ...
        console.log(`[VnExpress Travel] Successfully collected ${detailedItems.length} items`);
