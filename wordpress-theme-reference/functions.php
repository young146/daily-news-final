 ...
 * 
 * 🔧 2026-01-28 개선사항:
 *    - 날씨/환율: 매일 아침 6시(베트남 시간)에 1회만 업데이트
 *    - 캐시 TTL: 24시간 (하루 1회 갱신이므로 충분)
 *    - 에러 로깅 추가 (디버깅 용이)
 *    - API 호출 대폭 감소: 하루 144회 → 1회
 * 
 * ⚠️ WordPress Cron 주의사항:
 *    WordPress Cron은 방문자가 있어야 실행됩니다 (Pseudo-cron).
 *    안정적인 실행을 위해 서버 cron 설정을 권장합니다:
 *    0 6 * * * curl -s https://chaovietnam.co.kr/wp-cron.php > /dev/null
 *    (매일 아침 6시 실행 - 서버 시간 확인 필요)
 */
...
function cache_homepage_sections_data() {
    ...
    foreach ($sections as $section) {
        ...
        set_transient('home_section_' . $section['key'], $posts_data, 15 * MINUTE_IN_SECONDS);
    }
}
...
function update_weather_data_background() {
    ...
    foreach ($cities as $city) {
        ...
        if (is_wp_error($response)) {
            // 에러 로깅
            error_log('[ChaovietnamCache] Weather API failed for ' . $city['name'] . ': ' . $response->get_error_message());
        } else {
    ...
    // 24시간 캐시 (하루 1회 갱신)
    set_transient('weather_data', $weather_data, DAY_IN_SECONDS);
    
    error_log('[ChaovietnamCache] Weather data updated at ' . current_time('Y-m-d H:i:s'));
    return $weather_data;
}
...
function update_exchange_rates_background() {
    ...
    if (is_wp_error($response)) {
        // 에러 로깅
        error_log('[ChaovietnamCache] Exchange API failed: ' . $response->get_error_message());
    } else {
        $body = json_decode(wp_remote_retrieve_body($response), true);
    ...
    // 24시간 캐시 (하루 1회 갱신)
    set_transient('exchange_rates', $exchange_data, DAY_IN_SECONDS);
    
    error_log('[ChaovietnamCache] Exchange rates updated at ' . current_time('Y-m-d H:i:s'));
    return $exchange_data;
}


// ========================================
// 3-3. Cron 작업 등록 (매일 아침 6시 베트남 시간)
// ========================================

/**
 * 다음 아침 6시(베트남 시간) 타임스탬프 계산
 */
function get_next_6am_vietnam_timestamp() {
    // 베트남 시간대 (UTC+7)
    $vietnam_tz = new DateTimeZone('Asia/Ho_Chi_Minh');
    $now = new DateTime('now', $vietnam_tz);
    
    // 오늘 아침 6시
    $today_6am = new DateTime('today 06:00:00', $vietnam_tz);
    
    // 이미 6시가 지났으면 내일 6시
    if ($now > $today_6am) {
        $today_6am->modify('+1 day');
    }
    
    return $today_6am->getTimestamp();
}

function schedule_daily_news_cache_update() {
    // 날씨 업데이트: 매일 아침 6시 (베트남 시간)
    if (!wp_next_scheduled('cron_update_weather')) {
        wp_schedule_event(get_next_6am_vietnam_timestamp(), 'daily', 'cron_update_weather');
    }
    
    // 환율 업데이트: 매일 아침 6시 (베트남 시간)
    if (!wp_next_scheduled('cron_update_exchange')) {
        wp_schedule_event(get_next_6am_vietnam_timestamp(), 'daily', 'cron_update_exchange');
    }
}
...
/**
 * 기존 Cron 스케줄 정리 (10분마다 실행되던 것 제거)
 * 플러그인 활성화 시 또는 수동으로 한 번 실행 필요
 */
function cleanup_old_cron_schedules() {
    // 기존 10분마다 실행되던 스케줄 제거
    $weather_timestamp = wp_next_scheduled('cron_update_weather');
    if ($weather_timestamp) {
        wp_unschedule_event($weather_timestamp, 'cron_update_weather');
    }
    
    $exchange_timestamp = wp_next_scheduled('cron_update_exchange');
    if ($exchange_timestamp) {
        wp_unschedule_event($exchange_timestamp, 'cron_update_exchange');
    }
}
// 관리자 페이지 로드 시 기존 Cron 정리 (자동 실행)
add_action('admin_init', 'cleanup_old_cron_schedules');
